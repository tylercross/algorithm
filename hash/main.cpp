 ...
 //   HEAD = new LinkedList ;
  //  HEAD->insertItem(A);
   // HEAD->printList();
    ...
    //bool flag=HEAD->removeItem("Banana");
    // HEAD->printList();
    //if(flag) HEAD->printList();

    // Create a Hash Table of 13 Linked List elements.
    HashTable table;//因为构造函数声明中有可以用到的参数，可以成功调用用户唯一定义的
    //构造函数，同时由于用户自定义构造函数，系统将不会自动产生构造函数
